...
class ExternalNode(gpi.NodeAPI):
    ...
    def compute(self):
        ...
        if ( compute and (ZeroFill_factor_value > 0 or doHanning1 or doHanning2) ):
            ...
            if doHanning1:
                # zero-fill data
    #            kSpace = self.fft3D( np.array(cropped_data, dtype=np.complex64), dir=1)
    #            out_dims_fft = [ ZeroFill_factor * cropped_data.shape[0], ZeroFill_factor * cropped_data.shape[1], ZeroFill_factor * cropped_data.shape[2] ]
    #            ZeroFilled_data = np.abs( self.fft3D( kSpace, dir=0, out_dims_fft=out_dims_fft) )

                # zero-fill data
                kSpace = self.fft3D( np.array(data, dtype=np.complex64), dir=1)
                ...
                if doHanning2:
                    ...
                    Hann2_win = kaiser2D.window2(kSpace.shape[-2:], Hann2_taper, Hann2_width)
                ...
                if doHanning1:
                    ...
                    Hann1_win = kaiser2D.window1(kSpace.shape[-3], Hann1_taper, Hann1_width, stopVal=Hann1_stopVal)
                    self.log.node("Hann1 shape: "+str(Hann1_win.shape))
                    Hann1_winTile = Hann1_win[:,np.newaxis]
                    Hann1_winTile = Hann1_winTile[:,:,np.newaxis]
                    Hann1_winTile = np.tile(Hann1_winTile,(1,kSpace.shape[-2],kSpace.shape[-1]))
                    self.log.node("Hann1 tiled shape: "+str(Hann1_winTile.shape))
                    kSpace *= Hann1_winTile

                out_dims_fft = [ ZeroFill_factor * data.shape[0], ZeroFill_factor * data.shape[1], ZeroFill_factor * data.shape[2] ]
                ZeroFilled_data = np.abs( self.fft3D( kSpace, dir=fft_dir, out_dims_fft=out_dims_fft) )
            else:
                # data dimensions
                nx = data.shape[-1]
                ny = data.shape[-2]
                ...
                out_dims_fft = [ extra_dim2, extra_dim1, ZeroFill_factor * ny, ZeroFill_factor * nx]
                    
                # inverse FFT data in-plane only
                kSpace = kaiser2D.fft2D( np.array(data, dtype=np.complex64), dir=1)

                if doHanning2:
                    self.log.node('Applying 2D Hann filter (in plane)')
                    Hann2_win = kaiser2D.window2(kSpace.shape[-2:], Hann2_taper, Hann2_width)
                    for idx_dim1 in range(extra_dim1):
                        for idx_dim2 in range(extra_dim2):
                            kSpace[idx_dim2,idx_dim1,...] *= Hann2_win

                ZeroFilled_data = kaiser2D.fft2D( kSpace, dir=fft_dir, out_dims_fft=out_dims_fft)

            self.setData('Zero filled data', ZeroFilled_data.squeeze())
            if doHanning2 and Hann2_win is not None:
                self.setData('Hann 2D window', Hann2_win)            
            if doHanning1 and Hann1_win is not None:
                self.setData('Hann 1D window', Hann1_win)            
    ...
    def fft3D(self, data, dir=0, out_dims_fft=[]):
        # data: np.complex64
